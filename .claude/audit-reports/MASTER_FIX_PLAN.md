...
- [x] ~~`listUsers()` unpaginated in two places~~ — fixed 2026-09-04 via a shared paginating helper (`lib/supabase/listAllUsers.ts`), used in both `app/api/webhooks/stripe/route.ts` and `app/dashboard/settings/page.tsx`. Provider-agnostic (settings/page.tsx doesn't touch Stripe at all), so this didn't need to wait for the provider swap.
...
- [x] ~~`inviteMember` sends a real invite email but no redemption flow exists~~ — **stale, corrected 2026-09-04**: redemption exists and works for password signups (`on_auth_user_created` + `supabase/migrations/20260901160000_invite_redemption_on_signup.sql`); Phase 17 already fixed a stale comment saying otherwise, this checklist line just hadn't been updated to match. The real gap (invite redemption didn't work for Google OAuth signups) is fixed under the OAuth pass — see `lib/auth/redeemInviteForOAuthUser.ts`.
