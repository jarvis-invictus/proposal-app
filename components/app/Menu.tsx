...
export function Menu({
  ...
  // Outside-click (exempting the trigger, per the reopen-race fix) and scroll/resize-close —
  // shared with SelectMenu.tsx's identical logic via this hook. Escape stays here, separately:
  // it differs for a real reason (document-level here vs. SelectMenu's listbox-scoped handling
  // bundled with Tab/arrow-nav and a refocus), so it isn't part of the shared hook.
  useDismissOnOutsideOrInvalidate(open,()=>onClose&&onClose(),[ref,triggerRef]);
  React.useEffect(()=>{
    if(!open)return;
